            ...
            <div style={{ background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: '10px', padding: '12px 16px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '13px', color: 'var(--text)' }}>
                  {new Date(analytics.proximoPago.fecha).toLocaleDateString('es-AR', { day: '2-digit', month: 'long', year: 'numeric' })}
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
                  ...
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--border)' }}>
                      {['#', 'Fecha', 'Interés', 'Amortiz.', 'Total'].map(h => (
                        <th key={h} style={{ padding: '6px 10px', color: 'var(--muted2)', fontWeight: 400, textAlign: 'right' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {(showAllFlujos ? analytics.flujos : analytics.flujos.slice(0, 8)).map((f: any, i: number) => (
                      <tr key={i} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                        ...
                        <td style={{ padding: '7px 10px', color: 'var(--text2)', textAlign: 'right', whiteSpace: 'nowrap' }}>
        ...
        <>
          {hotMovers.length > 0 && (
            <div style={{ marginBottom: '28px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
                {hotMovers.map(a => (
                  <div key={a.ticker} onClick={() => setQuery(a.ticker)} style={{ background: a.variacion! > 0 ? 'rgba(16,185,129,0.08)' : 'rgba(244,63,94,0.08)', border: `1px solid ${a.variacion! > 0 ? 'rgba(16,185,129,0.2)' : 'rgba(244,63,94,0.2)'}`, borderRadius: '12px', padding: '16px', cursor: 'pointer' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '14px', color: 'var(--text)' }}>{a.ticker}</span>
                      {a.variacion! > 0 ? <TrendingUp size={16} color="var(--green)" /> : <TrendingDown size={16} color="var(--red)" />}
                    </div>
                    <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '18px', fontWeight: 500, color: 'var(--text)', marginBottom: '4px' }}>{'$' + (a.precio ?? 0).toLocaleString('es-AR', { maximumFractionDigits: 2 })}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div className="card">
              ...
              {acciones.map((a, i) => (
                <div key={a.ticker} onClick={() => setQuery(a.ticker)} style={{ display: 'grid', gridTemplateColumns: '80px 1fr auto', alignItems: 'center', padding: '10px 0', borderBottom: i < acciones.length - 1 ? '1px solid var(--border)' : 'none', cursor: 'pointer' }}>
            ...
            <div className="card">
              ...
              {bonos.map((b, i) => (
                <div key={b.ticker} onClick={() => setQuery(b.ticker)} style={{ display: 'grid', gridTemplateColumns: '80px 1fr auto', alignItems: 'center', padding: '10px 0', borderBottom: i < bonos.length - 1 ? '1px solid var(--border)' : 'none', cursor: 'pointer' }}>
