...
function parseIOLTipoMov(tipoMov: string): { tipo: 'compra' | 'venta' | 'dividendo' | 'deposito' | 'retiro'; ticker: string } | null {
  ...
  if (tipoMov.startsWith('Pago de Dividendos(')) { const inner = tipoMov.slice('Pago de Dividendos('.length, -1).replace(/ US\$$/, '').trim(); return { tipo: 'dividendo', ticker: inner }; }
...
function parseIOL(html: string, mep: number): Omit<OpImportada, 'isDuplicate' | 'selected'>[] {
  ...
  const rawRows = allRows.slice(1).map(row => { const cells = Array.from(row.querySelectorAll('td')).map(td => td.textContent?.trim() || ''); return Object.fromEntries(headers.map((h, i) => [h, cells[i] ?? ''])); }).filter(r => r['Tipo Mov.']);
  ...
  for (const [boleto, rows] of Array.from(groups.entries())) {
    ...
    if (esCash) {
      ...
      monto_usd = montoARS / mep; moneda = 'ARS'; fecha = parseIOLDate(row['Concert.'] || row['Liquid.']);
    } else {
...
function parseBalanz(buffer: ArrayBuffer, mep: number): Omit<OpImportada, 'isDuplicate' | 'selected'>[] {
  ...
  const boletoGroups = new Map<string, any[]>(); const dividendRows: any[] = [];
  ...
  for (const [boleto, rows] of Array.from(boletoGroups.entries())) {
    const { _tipo: tipo, _ticker: ticker } = rows[0]; const tipoInstr = rows[0]['Tipo de Instrumento'] || null;
  ...
  for (const [key, rows] of Array.from(divGrouped.entries())) {
    const usdRow = rows.find(r => { const m = String(r['Moneda'] || ''); return (m.includes('Dólar') || m.includes('Dollar')) && (r['Importe'] || 0) > 0; }); if (!usdRow) continue;
    const ticker = usdRow['_ticker']; const fecha = parseXLSXDate(usdRow['Concertacion']); const importe = usdRow['Importe']; if (!fecha || !importe) continue;
...
function TabPosiciones({ posiciones, efectivoUSD, mep }: { posiciones: PosicionCompleta[]; efectivoUSD: number; mep: number }) {
  ...
  // Columnas a mostrar en mobile (menos columnas)
  const colsMobile = ['Activo', `Precio`, 'P&L %', 'Var. Hoy'];
  const colsDesktop = [`Activo`, `Precio (${monedaLabel})`, 'Cantidad', `Costo Prom. (${monedaLabel})`, `Valor Actual (${monedaLabel})`, `P&L (${monedaLabel})`, 'P&L %', 'Var. Hoy', 'Tipo', 'Broker'];
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'DM Mono, monospace', fontSize: '13px' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface2)' }}>
              {(isMobile ? colsMobile : colsDesktop).map(h => (
            ...
            {efectivoUSD > 0 && (
              <tr style={{ borderBottom: '1px solid var(--border)', background: 'rgba(6,182,212,0.03)' }}>
                ...
                {isMobile ? <>
                  ...
                  <td colSpan={2} style={{ padding: '10px 12px', textAlign: 'right', color: 'var(--muted)' }}>—</td>
                </> : <>
                  <td colSpan={3} />
                  <td style={{ padding: '10px 12px', textAlign: 'right', color: '#06b6d4', fontWeight: 500 }}>{verUSD ? fmtUSD(efectivoUSD) : fmtARS(efectivoUSD * mep)}</td>
          ...
          <tfoot>
            <tr style={{ borderTop: '2px solid var(--border)', background: 'var(--surface2)' }}>
              <td style={{ padding: '10px 12px', fontFamily: 'Syne, sans-serif', fontWeight: 700, color: 'var(--text)', fontSize: '13px' }}>Total</td>
              {isMobile ? <>
  ...
  return (
    <div className="card">
      <div style={{ marginBottom: '12px' }}>
        <div className="label-xs" style={{ marginBottom: '4px' }}>🗺️ Mapa de posiciones</div>
        <div style={{ fontSize: '12px', color: 'var(--muted)' }}>Tamaño = valor · Color = P&L</div>
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '12px' }}>
        <div className="card" style={{ borderColor: totalNoRealizada >= 0 ? 'rgba(34,197,94,0.2)' : 'rgba(244,63,94,0.2)' }}>
          ...
          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '20px', fontWeight: 700, color: colorV(totalNoRealizada) }}>{totalNoRealizada >= 0 ? '+' : ''}{fmtUSD(totalNoRealizada)}</div>
        ...
        <div className="card" style={{ borderColor: totalRealizada >= 0 ? 'rgba(34,197,94,0.2)' : 'rgba(244,63,94,0.2)' }}>
          ...
          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '20px', fontWeight: 700, color: colorV(totalRealizada) }}>{totalRealizada >= 0 ? '+' : ''}{fmtUSD(totalRealizada)}</div>
    ...
    const build = async () => {
      ...
      try {
        ...
        if (puntos.length > 0 && valorActualIol > 0) {
          ...
          if (ultimoValor > 0) { const factor = valorActualIol / ultimoValor; puntos.forEach(p => { p.valor = Math.round(p.valor * factor * 100) / 100; p.rendimiento = p.invertido > 0 ? +((p.valor - p.invertido) / p.invertido * 100).toFixed(2) : 0; }); }
        }
          ...
          <tbody>
            {[...operaciones].reverse().map((op, i) => (
              <tr key={op.id} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                ...
                <td style={{ padding: '9px 12px', color: 'var(--text)', fontWeight: 500 }}>{fmtUSD(op.monto_usd)}</td>
                {!isMobile && <>
...
function ModalImportarBroker({ operacionesExistentes, mep, onClose, onImport }: { operacionesExistentes: Operacion[]; mep: number; onClose: () => void; onImport: (ops: Omit<Operacion, 'id'>[]) => Promise<void> }) {
  ...
  const handleFile = useCallback(async (file: File) => { setLoading(true); setError(''); setOps([]); try { let parsed: Omit<OpImportada, 'isDuplicate' | 'selected'>[] = []; if (broker === 'IOL') { const text = await file.text(); parsed = parseIOL(text, mep); } else { const buffer = await file.arrayBuffer(); parsed = parseBalanz(buffer, mep); } if (!parsed.length) { setError('No se encontraron operaciones.'); setLoading(false); return; } setOps(parsed.map(op => ({ ...op, isDuplicate: isDuplicateOp(op, operacionesExistentes), selected: !isDuplicateOp(op, operacionesExistentes) }))); } catch (e: any) { setError(e?.message || 'Error al leer el archivo.'); } setLoading(false); }, [broker, mep, operacionesExistentes]);
              ...
              <div style={{ borderRadius: '10px', overflow: 'hidden', border: '1px solid var(--border)', marginBottom: '16px' }}>
                ...
                <div style={{ maxHeight: '300px', overflowY: 'auto', overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'DM Mono, monospace', fontSize: '12px' }}>
                    ...
                    <tbody>{ops.map(op => (<tr key={op.importId} onClick={() => toggleOne(op.importId)} style={{ borderBottom: '1px solid var(--border)', background: op.isDuplicate ? 'rgba(245,158,11,0.03)' : op.selected ? 'rgba(124,58,237,0.04)' : 'transparent', cursor: op.isDuplicate ? 'default' : 'pointer', opacity: op.isDuplicate ? 0.6 : 1 }}><td style={{ padding: '8px 12px', textAlign: 'center' }}><input type="checkbox" checked={op.selected} disabled={op.isDuplicate} onChange={() => toggleOne(op.importId)} style={{ width: '14px', height: '14px', cursor: op.isDuplicate ? 'default' : 'pointer', accentColor: 'var(--violet)' }} onClick={e => e.stopPropagation()} /></td><td style={{ padding: '8px 12px', color: 'var(--text2)', whiteSpace: 'nowrap' }}>{new Date(op.fecha + 'T12:00:00').toLocaleDateString('es-AR', { day: '2-digit', month: 'short', year: '2-digit' })}</td><td style={{ padding: '8px 12px' }}><span style={{ color: TIPO_COLORS_OP[op.tipo] || 'var(--text)', fontWeight: 600, textTransform: 'capitalize' }}>{op.tipo}</span></td><td style={{ padding: '8px 12px', fontFamily: 'Syne, sans-serif', fontWeight: 700, color: 'var(--text)' }}>{op.ticker}</td><td style={{ padding: '8px 12px', color: 'var(--text)', fontWeight: 600 }}>{fmtUSD(op.monto_usd)}</td><td style={{ padding: '8px 12px' }}>{op.isDuplicate ? <span style={{ fontSize: '10px', color: 'var(--amber)', background: 'rgba(245,158,11,0.12)', borderRadius: '4px', padding: '2px 6px' }}>duplicado</span> : <span style={{ fontSize: '10px', color: 'var(--green)', background: 'rgba(34,197,94,0.12)', borderRadius: '4px', padding: '2px 6px' }}>nueva</span>}</td></tr>))}</tbody>
                  </table>
                </div>
              </div>
              <div style={{ fontSize: '11px', color: 'var(--muted)', fontFamily: 'DM Mono, monospace', marginBottom: '16px' }}>* ARS convertido a USD usando MEP (${mep.toLocaleString('es-AR')}).</div>
...
export default function PortfolioPage() {
  ...
  return (
    <div style={{ maxWidth: '1100px' }}>
      ...
      {operaciones.length === 0 ? (<EmptyState onAdd={() => setShowModal(true)} onImport={() => setShowImport(true)} />) : (
        <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
              {netoCaucionesUSD !== null && (
                <div className="card" style={{ borderColor: netoCaucionesUSD >= 0 ? 'rgba(16,185,129,0.2)' : 'rgba(244,63,94,0.2)' }}>
                  ...
                  <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '20px', fontWeight: 600, color: netoCaucionesUSD >= 0 ? 'var(--green)' : 'var(--red)', marginBottom: '4px' }}>{netoCaucionesUSD >= 0 ? '+' : ''}{fmtUSD(netoCaucionesUSD)}</div>
                  <div style={{ fontSize: '11px', color: 'var(--muted)', fontFamily: 'DM Mono, monospace' }}>P&L activos − costo cauciones · incluido en capital</div>
                </div>
              )}
              {netoCaucionesARS !== null && (
                <div className="card" style={{ borderColor: netoCaucionesARS >= 0 ? 'rgba(16,185,129,0.2)' : 'rgba(244,63,94,0.2)' }}>
                  ...
                  <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '20px', fontWeight: 600, color: netoCaucionesARS >= 0 ? 'var(--green)' : 'var(--red)', marginBottom: '4px' }}>{netoCaucionesARS >= 0 ? '+' : ''}{fmtARS(netoCaucionesARS)}</div>
                  <div style={{ fontSize: '11px', color: 'var(--muted)', fontFamily: 'DM Mono, monospace' }}>≈ {fmtUSD(Math.abs(netoCaucionesARS) / mep)} USD · incluido en capital</div>
                </div>
              )}
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: '4px', marginBottom: '20px', background: 'var(--surface2)', borderRadius: '12px', padding: '4px', overflowX: 'auto' }}>
            {[{ key: 'posiciones' as Tab, label: 'Posiciones', icon: '📋' },{ key: 'mapa' as Tab, label: 'Mapa', icon: '🗺️' },{ key: 'distribucion' as Tab, label: 'Dist.', icon: '🥧' },{ key: 'performance' as Tab, label: 'Perf.', icon: '🏆' },{ key: 'historial' as Tab, label: 'Historial', icon: '📈' },{ key: 'operaciones' as Tab, label: 'Ops.', icon: '📝' }].map(t => (
