...
function CaucionesTab({ cauciones, periodos, monedaActiva, onAdd, onRenovar, onDelete }: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    await onAdd({ descripcion: form.descripcion, monto: Number(form.monto), tna: Number(form.tna), plazo: Number(form.plazo), fechaInicio: form.fechaInicio, moneda: monedaActiva });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div className="card">
        <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '13px', fontWeight: 700, color: 'var(--text)', marginBottom: '16px' }}>
          Nueva Caución Tomadora <span style={{ color: monedaActiva === 'ARS' ? 'var(--amber)' : 'var(--green)', fontSize: '11px', fontFamily: 'DM Mono, monospace' }}>· {monedaActiva}</span>
        </div>
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '12px', marginBottom: '14px' }}>
            <div style={{ gridColumn: 'span 2' }}><label style={ls}>Descripción</label><input className="input-field" type="text" placeholder="Ej: Caución BYMA" value={form.descripcion} onChange={e => set('descripcion', e.target.value)} /></div>
          ...
          {form.monto && form.tna && form.plazo && (
            <div style={{ marginBottom: '12px', padding: '10px 14px', background: 'rgba(124,58,237,0.06)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: '8px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
              <div><span style={{ fontSize: '11px', color: 'var(--muted)' }}>Interés estimado: </span><span style={{ fontFamily: 'DM Mono, monospace', fontWeight: 600, color: 'var(--red)' }}>{fmtM(calcInteresPeriodo(Number(form.monto), Number(form.tna), Number(form.plazo)), monedaActiva)}</span></div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'DM Mono, monospace', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface2)' }}>
                  ...
                  {['Descripción','Monto','TNA','Plazo','Vencimiento','Días','Renovac.','Int. período','Int. total','Estado',''].map((h, i) => (
                    <th key={i} style={{ padding: '10px 14px', textAlign: h === 'Descripción' ? 'left' : 'right', fontSize: '11px', fontWeight: 700, fontFamily: 'Syne, sans-serif', color: 'var(--muted2)', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <>
                      <tr key={c.id} style={{ borderBottom: (isRenovando || isExpanded) ? 'none' : '1px solid var(--border)' }}
                        ...
                        <td style={{ padding: '12px 14px', textAlign: 'right', color: 'var(--amber)', fontWeight: 500 }}>{fmtM(c.monto, monedaActiva)}</td>
                        ...
                        <td style={{ padding: '12px 14px', textAlign: 'right', color: 'var(--red)' }}>{fmtM(costoActual, monedaActiva)}</td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', color: 'var(--red)', fontWeight: 600 }}>{fmtM(costoHist + costoActual, monedaActiva)}</td>
                        ...
                        <td style={{ padding: '12px 14px' }}>
                          <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                            <button onClick={() => startRenovar(c)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: isRenovando ? 'var(--violet-light)' : 'var(--muted)', padding: '3px', display: 'flex', alignItems: 'center' }}
                            ...
                            {confirmDelete === c.id ? (
                              ...
                              <button onClick={() => setConfirmDelete(c.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', padding: '3px', display: 'flex', alignItems: 'center' }}
                              ...
                              <tbody>
                                {historial.map((p, idx) => (
                                  <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                                    ...
                                    <td style={{ padding: '8px 14px', textAlign: 'right', color: 'var(--muted2)' }}>{fmtM(p.monto, monedaActiva)}</td>
                                    ...
                                    <td style={{ padding: '8px 14px', textAlign: 'right', color: 'var(--red)' }}>{fmtM(p.intereses, monedaActiva)}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
...
function ActivosTab({ activos, monedaActiva, onAdd, onUpdate, onDelete }: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    await onAdd({ ticker: form.ticker.toUpperCase(), tipo: form.tipo, cantidad: Number(form.cantidad), precioCompra: Number(form.precioCompra), precioActual: Number(form.precioActual), moneda: monedaActiva });
    setForm(EMPTY); setSaving(false);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div className="card">
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '12px', marginBottom: '14px' }}>
            ...
            <div><label style={ls}>Ticker</label><input className="input-field" type="text" placeholder={monedaActiva === 'ARS' ? 'S31O5, AL30...' : 'NVDAD, AL30D...'} value={form.ticker} onChange={e => set('ticker', e.target.value.toUpperCase())} required style={{ fontFamily: 'DM Mono, monospace' }} /></div>
            ...
            <div><label style={ls}>P. compra ({monedaActiva})</label><input className="input-field" type="number" step="0.0001" placeholder={monedaActiva === 'ARS' ? '1050.50' : '18.50'} value={form.precioCompra} onChange={e => set('precioCompra', e.target.value)} required style={{ fontFamily: 'DM Mono, monospace' }} /></div>
            <div><label style={ls}>P. actual ({monedaActiva})</label><input className="input-field" type="number" step="0.0001" placeholder={monedaActiva === 'ARS' ? '1082.00' : '19.20'} value={form.precioActual} onChange={e => set('precioActual', e.target.value)} required style={{ fontFamily: 'DM Mono, monospace' }} /></div>
          </div>
      ...
      {activos.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(170px, 1fr))', gap: '12px' }}>
          {[
            ...
            { label: 'P&L NO REALIZADO', value: fmtM(pnlAbierto, monedaActiva), color: pnlAbierto >= 0 ? 'var(--green)' : 'var(--red)' },
            { label: 'P&L REALIZADO', value: fmtM(pnlCerrado, monedaActiva), color: pnlCerrado >= 0 ? 'var(--green)' : 'var(--red)' },
          ].map(card => (
            <div key={card.label} className="card">
              <div className="label-xs" style={{ marginBottom: '8px' }}>{card.label}</div>
          ...
          <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <button onClick={handleRefresh} disabled={refreshing}
                style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '8px', padding: '6px 14px', cursor: refreshing ? 'not-allowed' : 'pointer', color: 'var(--text2)', fontSize: '12px', fontFamily: 'Syne, sans-serif', fontWeight: 600, opacity: refreshing ? 0.6 : 1 }}
                ...
                {abiertas.map(a => {
                  ...
                  return (
                    <tr key={a.id} style={{ borderBottom: '1px solid var(--border)' }}
                      ...
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: 'var(--muted2)' }}>{fmtM(a.precioCompra, monedaActiva, 4)}</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right' }}>
                        {isEditing ? editInput(editPrecio, setEditPrecio) : <span style={{ color: 'var(--text)' }}>{fmtM(a.precioActual, monedaActiva, 4)}</span>}
                      </td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: 'var(--muted2)' }}>{fmtM(a.precioCompra * a.cantidad, monedaActiva)}</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: 'var(--amber)' }}>{fmtM(a.precioActual * a.cantidad, monedaActiva)}</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: pnl >= 0 ? 'var(--green)' : 'var(--red)', fontWeight: 600 }}>{fmtM(pnl, monedaActiva)}</td>
                      ...
                      <td style={{ padding: '12px 14px' }}>
                        {isSelling ? (
                          <div style={{ display: 'flex', gap: '6px', alignItems: 'center', justifyContent: 'flex-end' }}>
                            ...
                            <button onClick={async () => { if (!sellData.precio) return; await onUpdate(a.id, { precioVenta: Number(sellData.precio), fechaVenta: sellData.fecha }); setSelling(null); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--green)', padding: '3px', display: 'flex', alignItems: 'center' }}><Check size={14} /></button>
                          ...
                          <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                            <button onClick={async () => { await onUpdate(a.id, { precioActual: Number(editPrecio) }); setEditing(null); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--green)', padding: '3px', display: 'flex', alignItems: 'center' }}><Check size={14} /></button>
      ...
      {cerradas.length > 0 && (
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)' }}><div className="label-xs">Posiciones cerradas</div></div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'DM Mono, monospace', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface2)' }}>
                ...
                {cerradas.map(a => {
                  ...
                  return (
                    <tr key={a.id} style={{ borderBottom: '1px solid var(--border)', opacity: 0.75 }}
                      ...
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: 'var(--muted2)' }}>{fmtNum(a.cantidad, 2)}</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: 'var(--muted2)' }}>{fmtM(a.precioCompra, monedaActiva, 4)}</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: 'var(--text)' }}>{fmtM(a.precioVenta!, monedaActiva, 4)}</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: pnl >= 0 ? 'var(--green)' : 'var(--red)', fontWeight: 700 }}>{fmtM(pnl, monedaActiva)}</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: pnlPct >= 0 ? 'var(--green)' : 'var(--red)', fontWeight: 700 }}>{fmtPct(pnlPct)}</td>
      ...
      {activos.length === 0 && (
        <div className="card" style={{ textAlign: 'center', padding: '60px' }}>
          ...
          <div style={{ fontSize: '13px', color: 'var(--muted2)' }}>{monedaActiva === 'ARS' ? 'Agregá LECAPs, bonos ARS u otros activos en pesos.' : 'Agregá CEDEARs, bonos USD u otros activos en dólares.'}</div>
        </div>
      )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '12px' }}>
        {[
          { label: 'P&L NO REALIZADO', value: fmtM(pnlNoRealizado, monedaActiva), sub: `${abiertas.length} posición${abiertas.length !== 1 ? 'es' : ''} abierta${abiertas.length !== 1 ? 's' : ''}`, color: pnlNoRealizado >= 0 ? 'var(--green)' : 'var(--red)' },
          { label: 'P&L REALIZADO', value: fmtM(pnlRealizado, monedaActiva), sub: `${cerradas.length} posición${cerradas.length !== 1 ? 'es' : ''} cerrada${cerradas.length !== 1 ? 's' : ''}`, color: pnlRealizado >= 0 ? 'var(--green)' : 'var(--red)' },
          ...
          { label: 'COSTO CAUCIONES', value: fmtM(totalCostoCauciones, monedaActiva), sub: `${cauciones.length} caución${cauciones.length !== 1 ? 'es' : ''} en ${monedaActiva}`, color: 'var(--red)' },
          { label: 'RENDIMIENTO NETO', value: fmtM(rendimientoNeto, monedaActiva), sub: fmtPct(rendimientoNetoPct), color: rendimientoNeto >= 0 ? 'var(--green)' : 'var(--red)' },
        ].map(card => (
          <div key={card.label} className="card">
            <div className="label-xs" style={{ marginBottom: '8px' }}>{card.label}</div>
      ...
      {abiertas.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div className="card">
            ...
            <div style={{ fontSize: '11px', color: 'var(--muted2)', marginBottom: '16px' }}>Ganancia o pérdida de cada posición abierta, sin costo de financiamiento.</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {(['USD', 'ARS'] as Moneda[]).map(m => {
            ...
            return (
              <button key={m} onClick={() => setMonedaActiva(m)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 20px', borderRadius: '10px', border: `2px solid ${active ? (m === 'USD' ? '#10b981' : 'var(--amber)') : 'var(--border)'}`, background: active ? (m === 'USD' ? 'rgba(16,185,129,0.08)' : 'rgba(245,158,11,0.08)') : 'var(--surface2)', cursor: 'pointer', transition: 'all 0.15s' }}>
                <span style={{ fontFamily: 'DM Mono, monospace', fontWeight: 800, fontSize: '16px', color: active ? (m === 'USD' ? '#10b981' : 'var(--amber)') : 'var(--muted2)' }}>{m}</span>
                <span style={{ fontSize: '10px', color: 'var(--muted)', fontFamily: 'DM Mono, monospace', marginTop: '2px' }}>{stats.caucs} cauc · {stats.acts} activos</span>
              </button>
            );
          })}
        </div>
